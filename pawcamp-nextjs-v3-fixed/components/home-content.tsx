    ...
    <div className="space-y-6 px-4 pb-6 pt-5">
      <section className="app-gradient soft-grid rounded-[34px] p-5 text-white shadow-lg">
        <div className="mb-6 flex items-start justify-between gap-3">
          <div>
            <p className="text-sm text-white/72">PawCamp V3</p>
            <h1 className="mt-2 text-[44px] leading-[1.02] font-semibold">Live campsite discovery built for pet travel.</h1>
            <p className="mt-3 max-w-[90%] text-sm leading-6 text-white/76">Real map browsing, geolocation, and fast dog-vs-cat filtering in one mobile-first experience.</p>
          </div>
      ...
      <section className="space-y-4">
        <Card className="bg-surface-muted">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-semibold">Why V3 feels more real</p>
              <p className="mt-1 text-sm leading-6 text-muted">The Discover page now uses a real Mapbox map, lets users opt into device location, and keeps the shortlist connected to the live map state.</p>
            </div>
          </div>
        </Card>
      </section>
