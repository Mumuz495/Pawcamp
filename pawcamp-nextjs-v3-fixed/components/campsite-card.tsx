...
export function CampsiteCard({ campsite }: { campsite: Campsite }) {
  ...
  return (
    <Link href={`/campsite/${campsite.slug}`} className="group block rounded-[30px] border border-border bg-surface shadow-soft transition hover:-translate-y-0.5 hover:shadow-lg">
      <div className="relative h-52 overflow-hidden rounded-t-[30px]">
        ...
        <div className="absolute bottom-4 left-4 right-4 text-white">
          ...
          <div className="flex items-end justify-between gap-3">
            ...
            <div className="rounded-full bg-black/25 px-3 py-2 text-sm font-medium backdrop-blur">★ {campsite.rating}</div>
          </div>
        </div>
      </div>
