...
function fitSummary(campsite: Campsite) {
  if (campsite.petPolicy.catFriendly && campsite.petPolicy.dogFriendly) return 'Strong fit for both dog and cat travel.';
  if (campsite.petPolicy.catFriendly) return 'Better than average for cat travel and quiet setups.';
  if (campsite.petPolicy.dogFriendly) return 'Best suited for dog-first camping trips.';
  return 'Check restrictions carefully before planning this trip.';
}
    ...
    <div className="pb-6">
      <div className="relative h-[360px] overflow-hidden rounded-b-[34px]">
        ...
        <div className="absolute left-4 right-4 top-4 flex items-center justify-between">
          ...
          <button onClick={() => toggleFavorite(campsite.id)} className="rounded-full bg-surface/92 p-2.5 text-foreground shadow-soft" aria-label="Toggle favorite">
        ...
        <div className="absolute bottom-5 left-4 right-4 text-white">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            <span className="inline-flex items-center gap-1 rounded-full bg-white/16 px-3 py-1.5 text-sm font-medium backdrop-blur">
              <Star size={14} className="fill-yellow-400 text-yellow-400" /> {campsite.rating} - {campsite.reviews}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-5 px-4 pt-5">
        <Card className="bg-surface-muted">
          <SectionHeader eyebrow="Pet fit snapshot" title={fitSummary(campsite)} subtitle="This section answers the core question before the user reads the rest of the page." />
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            {campsite.petPolicy.leashRequired ? <Pill>Leash required</Pill> : <Pill tone="warning">Leash flexibility</Pill>}
            {campsite.atmosphere.quiet ? <Pill tone="success">Quiet setting</Pill> : <Pill tone="warning">Busier setting</Pill>}
          </div>
